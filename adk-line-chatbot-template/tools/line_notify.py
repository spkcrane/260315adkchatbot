...
def notify_team_line(lead_summary: str) -> dict:
    ...
    if lead_hash in _notified_leads:
        print("[SKIP] notify_team_line — duplicate detected, already sent.")
        return {"status": "already_sent", "message": "Lead already notified. Skipped duplicate."}

    group_id = os.getenv("LINE_GROUP_ID")
    access_token = os.getenv("CHANNEL_ACCESS_TOKEN")

    if not group_id or group_id == "your_team_line_group_id":
    ...
    if not access_token or access_token == "your_line_access_token":
        print("[ERROR] notify_team_line — CHANNEL_ACCESS_TOKEN not configured.")
        return {"status": "error", "message": "CHANNEL_ACCESS_TOKEN is not configured."}
    ...
    payload = {
        "to": group_id,
        "messages": [{"type": "text", "text": lead_summary}],
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"status": "error", "message": f"LINE API error: {response.status_code} {response.text}"}
